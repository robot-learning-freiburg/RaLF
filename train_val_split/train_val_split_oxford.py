...
def extract_gps_poses(gps_file, ins_file, rtime_file, use_rtk=False):
    ...
    for radar_time in timestamps:
        gps_closest_indices.append(find_nearest_idx(gps_time, radar_time))
    ...
    gt_gps_poses = interpolate_ins_poses(ins_file, timestamps, timestamps[0], use_rtk=use_rtk)
...
if __name__ == "__main__":
    ...
    for seq_dir in sequences:
        if seq_dir == '2019-01-17-11-46-31-radar-oxford-10k':
            continue
        print(f"Processing sequence: {seq_dir}")
        sequence_path = os.path.join(dataroot, seq_dir)
        ...
        all_sequences += [seq_dir] * len(seq_poses)
        all_n += list(n)
        all_e += list(e)
