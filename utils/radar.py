...
def radar_polar_to_cartesian(azimuths: np.ndarray, fft_data: np.ndarray, radar_resolution: float,
    ...
    cart_img = ((cart_img - cart_img.min()) * (1/(cart_img.max() - cart_img.min()) * 255)).astype('uint8')
    return cart_img

if __name__ == '__main__':
    path = '../../data/oxford_radar_robotcar_dataset_sample_large/' \
           '2019-01-10-14-36-48-radar-oxford-10k-partial/radar/'
    file_ = ['../../data/oxford_radar_robotcar_dataset_sample_large/'
             '2019-01-10-14-36-48-radar-oxford-10k-partial/radar/1547131046353776.png']
    files = sorted(os.listdir(path))
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    out = cv2.VideoWriter('radar.mp4', fourcc, 15, (300, 300), 0)
    im_list = []
    for file in file_: #files:
        timestamps, azimuths, valid, fft_data, radar_resolution = load_radar(file)
        # timestamps, azimuths, valid, fft_data, radar_resolution = load_radar(os.path.join(path, file))
        radar_img = radar_polar_to_cartesian(azimuths, fft_data, radar_resolution, 0.25, 601)
        cv2.imshow("radar image", radar_img[:, :, 0])
        cv2.waitKey(0)
        im_list.append(radar_img[:, :, 0])
        cv2.imwrite("radar.png", np.round(radar_img[:, :, 0] * 255).astype(np.uint8))
        # print(radar_img.shape)

    for i in im_list:
        print(i.shape)
        out.write((i * 255).astype(np.uint8))
    out.release()
